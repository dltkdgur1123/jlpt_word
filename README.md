...
6. [실행 방법](#-실행-방법)
7. [Telegram AI Agent 운영 흐름](#-telegram-ai-agent-운영-흐름)
8. [매일 아침 Cron 운영](#-매일-아침-cron-운영)
9. [데이터 부족 감지·생성·검수](#-데이터-부족-감지생성검수)
10. [검증 및 복구 방식](#-검증-및-복구-방식)
11. [보안 및 신뢰성 원칙](#-보안-및-신뢰성-원칙)
12. [포트폴리오 요약](#-포트폴리오-요약)
...
# 🤖 Telegram AI Agent 운영 흐름

이 프로젝트는 Streamlit 대시보드에서 수동으로 실행할 수 있을 뿐 아니라, Telegram 기반 운영 봇과 연결해 자연어 지시로 생성·업로드·복구 상태를 확인할 수 있도록 운영했습니다.
...
예를 들어 아래와 같은 요청을 Telegram에서 처리할 수 있도록 운영 기준을 만들었습니다.

```text
오늘 JLPT 세트 업로드 상태 확인해줘
실패한 영상 복구해줘
예약 시간이 밀린 영상 정렬해줘
데이터 부족하면 보충하고 다시 생성해줘
```

AI Agent는 raw log를 그대로 전달하지 않고, 사용자가 알아야 하는 상태만 요약합니다.

| 요청 유형 | AI Agent가 확인하는 것 | 보고 방식 |
|---|---|---|
| 오늘 업로드 상태 | 생성 파일, 업로드 로그, YouTube 공개/예약 상태 | 완료/대기/복구 필요 |
| 생성 실패 | 후보 데이터 수량, 렌더링 로그, 영상 파일 존재 여부 | 어느 레벨/유형이 막혔는지 요약 |
| 업로드 실패 | OAuth, YouTube 제한, 썸네일/재생목록 경고 | 재인증 필요/자동 재시도/사용자 승인 필요 |
| 예약 정렬 | DAY 번호, publishAt, 실제 YouTube 상태 | 어떤 영상이 어느 시간에 맞춰졌는지 요약 |
| 데이터 부족 | 부족한 레벨과 단어/문법/표현 유형 | 자동 보충 후 재시도 또는 검수 필요 |

> AI Agent가 모든 것을 무조건 자동 처리하는 구조가 아니라, 상태 확인과 복구 판단을 보조하고 공개 영상 삭제·재업로드처럼 영향이 큰 작업은 사용자 승인 기준을 두는 방식으로 운영했습니다.

<br />

# 🌅 매일 아침 Cron 운영

반복 콘텐츠 운영은 매일 정해진 시간에 Cron으로 생성·업로드 작업이 실행되도록 구성했습니다. Cron은 사람이 매번 버튼을 누르지 않아도 정해진 루틴을 실행하고, Telegram 보고는 결과 중심으로 짧게 전달하는 방식으로 운영합니다.

```text
매일 아침 생성/검수 준비
→ 정해진 업로드 슬롯 확인
→ 필요한 영상 생성 또는 예약 상태 확인
→ 실패 항목은 복구 큐에 유지
→ 사용자에게 필요한 조치만 요약 보고
```

실제 운영에서는 채널별로 생성 시간과 업로드 시간을 분리합니다.
...
Cron 운영에서 중요한 기준은 **한 작업이 실패해도 다른 채널이나 다른 레벨이 불필요하게 스킵되지 않도록 분리하는 것**입니다.
...
각 흐름은 같은 운영 원칙을 공유하지만, 데이터와 업로드 상태는 분리해서 관리합니다.

<br />

# 🧪 데이터 부족 감지·생성·검수

자동화에서 가장 자주 막히는 지점은 영상 렌더링보다 **콘텐츠 후보 데이터 부족**입니다. 이 프로젝트는 후보가 부족할 때 단순 실패로 끝내지 않고, 부족한 단위를 좁혀 보충하고 다시 생성하는 흐름을 둡니다.

```text
데이터 게이트 확인
→ 부족한 레벨/유형 식별
→ 후보 데이터 자동 생성 또는 수동 보충
→ 의미·중복·표기 검수
→ 생성 게이트 재실행
→ 영상 생성/업로드 재개
```
...
| 표기 | 일본어, 히라가나, 한국어 뜻, 스페인어 뜻이 섞이지 않았는지 |
| 영상 적합성 | 화면에 들어갈 길이인지, TTS가 어색하지 않은지 |
| 공개 위험 | 공식 시험 문제 복제나 합격 보장처럼 오해될 표현이 없는지 |

데이터가 부족한 경우에도 전체 파이프라인을 멈추는 대신, 정확히 부족한 단위를 찾아 보충합니다.
...
이 방식은 커머스/운영 자동화 관점에서도 중요합니다. 단순히 에러를 출력하는 것이 아니라, **부족한 데이터 단위를 찾고, 생성하고, 검수한 뒤 다시 운영 흐름으로 되돌리는 구조**이기 때문입니다.
