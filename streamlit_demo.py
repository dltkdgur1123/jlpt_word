...
DEMO_MODE = True
DEMO_LOG_TEXT = """[DEMO] Step 0: DAY 세트 구성
[DEMO] Step 1: 카드 이미지 생성
[DEMO] Step 2: 일본어/한국어 TTS 생성
[DEMO] Step 3: 단어별 쇼츠 영상 생성
[DEMO] Step 4: DAY 영상 생성
[DEMO] Step 5: 썸네일 생성
[DEMO] Step 6: YouTube 예약 업로드
[DEMO] Step 7: Google Drive 백업
[DEMO] Step 8: 로컬 산출물 정리

[DEMO] 포트폴리오 모드에서는 실제 실행되지 않습니다.
"""


def blur_text(value, keep_start=3, keep_end=3, mask="•"):
    text = str(value)
    if not text or text == "-":
        return text
    ...
    return text[:keep_start] + (mask * max(len(text) - keep_start - keep_end, 4)) + text[-keep_end:]
...
def blur_path(value):
    ...
    if len(parts) <= 2:
        return blur_text(text, keep_start=2, keep_end=4)
...
def blur_link(value):
    ...
    return blur_text(text, keep_start=12, keep_end=8)
...
def get_upload_preview_rows(upload_day_map):
    ...
    for level, day in upload_day_map.items():
        ...
        preview_rows.append({
            ...
            "영상 경로": blur_path(video_path),
            "썸네일 경로": blur_path(thumbnail_path),
        })
...
def get_uploaded_video_options():
    ...
    for upload_key, entry in load_uploaded_log().items():
        ...
        options.append({
            ...
            "level": level,
            "day": day,
            "video_id": blur_text(video_id or "미기록", keep_start=2, keep_end=2),
...
def get_compilation_preview_rows(levels, start_day, end_day):
    ...
    for level in levels:
        ...
        rows.append({
            ...
            "Drive 영상 링크": blur_link(log_entry.get("drive_video_link") or "-"),
            "Drive 썸네일 링크": blur_link(log_entry.get("drive_thumbnail_link") or "-"),
            "영상 경로": blur_path(video_path),
            "썸네일 경로": blur_path(thumbnail_path),
...
st.set_page_config(
    page_title="JLPT / 비즈니스 쇼츠 대시보드 DEMO",
    page_icon="🎬",
    layout="wide",
)

st.title("JLPT / 비즈니스 쇼츠 대시보드 DEMO")
st.caption("포트폴리오용 시연 화면")
st.info(
    "이 페이지는 포트폴리오 데모 모드입니다. "
    "버튼 클릭 시 실제 영상 생성, 업로드, 예약 변경, Drive 백업은 실행되지 않습니다."
)

with st.sidebar:
    st.header("실행 모니터")
    st.caption("데모 모드에서도 실제 운영 화면과 같은 위치에 상태 패널을 배치했습니다.")
...
if refresh_schedule_button:
    st.info("데모 모드에서는 YouTube API를 호출하지 않고, 저장된 샘플 예약 정보만 표시합니다.")
...
if uploaded_video_options:
        ...
        [
            {
                ...
                "VIDEO ID": item.get("video_id") or blur_text("미기록", keep_start=1, keep_end=1),
            }
    ...
    st.write(
        f"현재 선택: `{blur_text(selected_reschedule['upload_key'], keep_start=4, keep_end=4)}` / "
        f"VIDEO ID: `{selected_reschedule.get('video_id') or blur_text('미기록', keep_start=1, keep_end=1)}`"
    )
    ...
    if reschedule_button:
        demo_publish_at = build_publish_at_from_kst(reschedule_date, reschedule_time)
        st.success("데모 모드: 예약 변경 시뮬레이션이 완료되었습니다.")
        st.code(
            f"[DEMO] {blur_text(selected_reschedule['upload_key'], keep_start=4, keep_end=4)} "
            f"예약 시간을 {demo_publish_at}로 변경",
            language="text",
        )
else:
...
if run_button:
    if not selected_levels:
        ...
        progress_bar.progress(1.0, text="DEMO 완료")
        status_placeholder.info("데모 모드에서는 실제 작업을 실행하지 않습니다.")
        live_log_placeholder.code(DEMO_LOG_TEXT, language="text")
        st.success("데모 실행 화면을 표시했습니다.")
        st.subheader("실행 로그")
        st.code(DEMO_LOG_TEXT, language="text")
...
if compilation_run_button:
    if not compilation_levels:
        ...
        comp_progress_bar.progress(1.0, text="DEMO 완료")
        comp_status_placeholder.info("데모 모드에서는 풀영상 생성/업로드를 실제로 실행하지 않습니다.")
        compilation_logs = (
            f"[DEMO] 풀영상 레벨: {', '.join(compilation_levels)}\n"
            f"[DEMO] 구간: DAY {int(compilation_start_day):03d} ~ DAY {int(compilation_end_day):03d}\n"
            f"[DEMO] 모드: {compilation_action_mode}\n"
            "[DEMO] 풀영상 생성 -> 업로드 -> Drive 백업 -> 로컬 삭제 흐름 시연"
        )
        comp_live_log_placeholder.code(compilation_logs, language="text")
        st.success("풀영상 데모 화면을 표시했습니다.")
